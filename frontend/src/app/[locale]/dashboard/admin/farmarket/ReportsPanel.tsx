...
export function ReportsPanel({ stats }: Props) {
  const t = useTranslations("admin.farmarket.reports");
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        />
        <ReportCard
          label={t("cards.cancelled")}
          value={String(stats.cancelled_count)}
          sub={t("cancelledSub", { pct: pct(stats.cancellation_rate) })}
        />
        <ReportCard
          label={t("cards.returnRate")}
          value={pct(stats.return_rate)}
          sub={t("returnedSub", { count: stats.returned_count })}
        />
      </div>
      ...
      <div className="rounded-lg border border-neutral-200 bg-white p-5 shadow-sm">
            ...
            return (
              <li key={st} className="flex items-center gap-3 text-xs">
                <span className="w-40 shrink-0 text-neutral-600">
                  {getOrderStatusLabel(st, tStatus)}
                </span>
        ...
        <p className="mt-4 text-[11px] text-neutral-400">
          {t("footer", {
            count: stats.orders_total,
            cancelRate: pct(stats.cancellation_rate),
            returnRate: pct(stats.return_rate),
          })}
        </p>
      </div>
    </div>
