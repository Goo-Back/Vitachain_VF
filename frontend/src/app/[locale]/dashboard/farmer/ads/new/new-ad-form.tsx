...
export function NewAdForm() {
  const t = useTranslations("farmer.ads.new.form");
  ...
  function errorMessage(code: string): string {
    const key = ERROR_KEYS.find((k) => code.startsWith(k));
    return key ? t(`errors.${key}`, { max: MAX_PHOTOS }) : t("errors.unexpected", { code });
  }
  ...
  function handleFiles(files: FileList | null) {
    ...
    for (const file of Array.from(files)) {
      if (next.length >= MAX_PHOTOS) {
        setPhotoError(t("tooManyPhotos", { max: MAX_PHOTOS }));
        break;
      }
      if (file.size > MAX_PHOTO_BYTES) {
        setPhotoError(t("photoTooLarge", { name: file.name, size: 2 }));
        continue;
      }
      if (!file.type.startsWith("image/")) {
        setPhotoError(t("notAnImage", { name: file.name }));
        continue;
      }
      ...
      <div>
        <p className={LABEL_CLASS}>
          ...
          <span className="font-normal text-neutral-500">
            {t("photosOptional", { max: MAX_PHOTOS, size: 2 })}
          </span>
        </p>
            ...
            {photos.map((p, i) => (
              <div key={p.url} className="relative">
                ...
                <button
                  ...
                  aria-label={t("removePhotoAriaLabel", { index: i + 1 })}
                  className="absolute -end-1.5 -top-1.5 flex h-5 w-5 items-center justify-center rounded-full bg-white text-neutral-500 shadow ring-1 ring-neutral-200 hover:text-red-600"
                >
                  <XIcon size={10} />
                </button>
              </div>
