...
export default async function SettingsPage() {
  const t = await getTranslations("farmer.settings.page");
  const tCommon = await getTranslations("farmer.common");
  const profile = await getServerProfile();

  const isVerified = profile?.verification_status === "VERIFIED";
  // Language autonyms — intentionally NOT translated: a language's name in
  // its own script (e.g. "العربية") stays constant regardless of the UI
  // locale, matching how language pickers conventionally render.
  const localeLabel =
      ...
      <Stagger className="space-y-6">
        ...
        >
          ...
          <dl className="mt-6 grid gap-4 border-t border-neutral-100 pt-6 sm:grid-cols-2">
            <Row label={t("email")} value={profile?.email ?? t("dash")} />
            <Row label={t("role")} value={profile?.role ?? t("dash")} />
            <Row label={t("language")} value={localeLabel} />
          </dl>
        </SectionCard>
        </StaggerItem>
