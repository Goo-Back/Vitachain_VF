...
interface Props {
  ...
  /** Scoped to farmer.parcels.detail.telemetry — supplies the "Aucune
   *  donnée.", "{count} pts" and chart aria-label copy without this plain
   *  component ever calling next-intl itself. */
  t: Translator;
}
  ...
  if (values.length === 0) {
    return (
      <figure className="rounded-lg border border-neutral-200 bg-white p-4 shadow-sm">
        ...
        <div className="mt-3 text-xs text-neutral-400">{t("noData")}</div>
      </figure>
    );
  }
  ...
  return (
    <figure className="rounded-lg border border-neutral-200 bg-white p-4 shadow-sm">
      ...
      <svg
        ...
        role="img"
        aria-label={t("chartAriaLabel", { title, value: last.toFixed(2), unit: u })}
      >
        <title>{title}</title>
        {hasMin && hasMax && (
          <rect
      ...
      <div className="mt-1 text-right text-xs tabular-nums text-neutral-500">
        {t("ptsSuffix", { count: values.length })}
      </div>
    </figure>
  );
}
