...
// i18n-KAT05
const METRIC_META: Record<Metric, { unit: string; step: number }> = {
  soil_moisture:     { unit: "%",     step: 1   },
  soil_temperature:  { unit: "°C",    step: 0.5 },
  soil_ph:           { unit: "",      step: 0.1 },
  soil_conductivity: { unit: "µS/cm", step: 50  },
  battery_level:     { unit: "%",     step: 1   },
};
...
export function ThresholdsSection({
  ...
  const t = useTranslations("farmer.parcels.detail.thresholds");
      ...
      <div className="overflow-hidden rounded-lg border border-neutral-200 bg-white shadow-sm">
        <table className="w-full text-sm">
          <thead className="bg-neutral-50 text-xs uppercase tracking-wide text-neutral-500">
            <tr>
              <th className="px-3 py-2 text-left">{t("metricHeader")}</th>
              <th className="px-3 py-2 text-right">{t("minHeader")}</th>
              <th className="px-3 py-2 text-right">{t("maxHeader")}</th>
              <th className="px-3 py-2 text-center">{t("enabledHeader")}</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((r) => {
              const meta = METRIC_META[r.metric];
              const label = t(`metrics.${r.metric}`);
                ...
                <tr key={r.metric} className="border-t border-neutral-100">
                  ...
                  <td className="px-3 py-2 text-right">
                    <input
                      ...
                      aria-label={t("minAriaLabel", { label })}
                    />
                    {meta.unit && (
                  ...
                  <td className="px-3 py-2 text-right">
                    <input
                      ...
                      aria-label={t("maxAriaLabel", { label })}
                    />
                    {meta.unit && (
                      <span className="ms-1 text-xs text-neutral-500">
                        {meta.unit}
                  ...
                  <td className="px-3 py-2 text-center">
                    <input
                      ...
                      aria-label={t("enableAriaLabel", { label })}
