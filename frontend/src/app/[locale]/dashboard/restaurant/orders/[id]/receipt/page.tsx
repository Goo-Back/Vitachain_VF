...
export default async function ReceiptPage({ params }: Props) {
  const t = await getTranslations("restaurant.orders.receipt");
  ...
  const createdAt = new Date(order.created_at).toLocaleDateString(intlLocale, {
    day: "numeric",
    month: "long",
    year: "numeric",
  });

  const columns = [t("colArticle"), t("colQuantity"), t("colUnitPrice"), t("colSubtotal")];
      ...
      >
        <header className="flex items-start justify-between border-b border-neutral-200 pb-6">
          <div>
            ...
            <p className="mt-1 text-xs text-neutral-500">
              {t("generatedOn", { date: new Date().toLocaleDateString(intlLocale) })}
            </p>
          </div>
        ...
        <section className="mt-6 grid gap-6 text-sm sm:grid-cols-2">
          ...
          <div>
            ...
            <p className="text-xs text-neutral-600">
              {order.delivery_notes ?? t("noSpecialNotes")}
            </p>
          </div>
        </section>
          ...
          <table className="min-w-full divide-y divide-neutral-200 text-sm">
            ...
            <tbody className="divide-y divide-neutral-100">
              {order.items.map((it, i) => (
                <tr key={it.id}>
                  <td className="py-3">
                    <p className="text-neutral-900">{t("articleLabel", { n: i + 1 })}</p>
        ...
        <section className="mt-6 flex justify-end">
          <dl className="w-full max-w-xs space-y-1.5 text-sm">
            <Row label={t("subtotalLabel")} value={Number(order.subtotal_mad)} />
            <Row label={t("logisticsLabel")} value={Number(order.logistics_fee_mad)} />
