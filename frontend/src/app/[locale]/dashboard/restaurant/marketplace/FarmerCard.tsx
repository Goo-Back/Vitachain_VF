 ...
 */
export async function FarmerCard({
  farmer,
  linkToProfile = true,
  basePath = "/dashboard/restaurant/marketplace",
}: {
  ...
  return (
    <div className="vc-card p-5">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <span className="mt-0.5 inline-flex items-center gap-1 text-xs text-emerald-600">
            <CheckCircleIcon size={12} /> {t("common.verifiedProducer")}
          </span>
      ...
      <dl className="mt-4 space-y-2 text-xs text-neutral-600">
        ...
        <div className="flex items-center gap-2">
          <span className="text-neutral-400">{t("common.memberSince")}</span>
          <span className="capitalize">{memberSince}</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-neutral-400">{t("common.activeAds")}</span>
      ...
      {linkToProfile && (
        <Link
          href={`${basePath}/farmer/${farmer.id}`}
          className="mt-4 inline-flex items-center gap-1 text-xs font-medium text-leaf-700 hover:underline"
        >
          {t("farmerCard.viewProfile")} <ArrowRightIcon size={12} className="rtl:-scale-x-100" />
        </Link>
      )}
    </div>
  );
