...
export default async function AdDetailPage({ params }: Props) {
  const t = await getTranslations("restaurant.marketplace");
  ...
  return (
    <div className="vc-fade-in">
      <PageHeader
        crumbs={[
          { label: t("common.catalogCrumb"), href: "/dashboard/restaurant/marketplace" },
          { label: ad.title },
        ]}
        eyebrow="FarMarket"
        title={ad.title}
        ...
        <section className="lg:col-span-2">
          ...
          <div className="vc-card mt-6 p-5">
            <h2 className="text-sm font-semibold text-neutral-900">
              {t("adDetail.description")}
            </h2>
            <p className="mt-2 whitespace-pre-line text-sm text-neutral-700">
              {ad.description || t("adDetail.noDescription")}
            </p>
          </div>
          ...
          <div className="vc-card p-5">
            {ad.is_featured && (
              <span className="mb-3 inline-flex items-center gap-1 rounded-full bg-amber-50 px-2 py-0.5 text-xs font-medium text-amber-700 ring-1 ring-inset ring-amber-200">
                <SparkleIcon size={12} /> {t("adDetail.featured")}
              </span>
            )}
            ...
            <p className="mt-3 text-[11px] text-neutral-400">
              {t("adDetail.validUntil", { date: expires })}
            </p>
          </div>
