...
export async function StarRating({
  ...
  const t = await getTranslations("restaurant.marketplace.starRating");
  const v = value ?? 0;
  return (
    <span className="inline-flex items-center gap-1.5" aria-label={t("ariaLabel", { value: v.toFixed(1) })}>
      ...
      {typeof count === "number" && (
        <span className="text-xs text-neutral-400">
          {count === 0 ? t("noReviews") : t("reviewsCount", { count })}
        </span>
      )}
    </span>
  );
