  ...
  return (
    <section className="mt-10">
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <h2 className="text-xl font-semibold">Capteurs — Mesures</h2>
            ...
            >
              {WINDOWS.map((w) => (
                <button
                  ...
                  type="button"
                  role="tab"
                  aria-selected={activeWindow === w.value}
  ...
  return (
    <div>
      ...
      <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-neutral-500">
        ...
        <span>Batterie {latest.battery_level} %</span>
        <span className="text-neutral-300">·</span>
        <span>Dernière lecture {relativeTs}</span>
        {latest.device_label && (
          <>
            <span className="text-neutral-300">·</span>
...
function EmptyState({ hasHistory }: { hasHistory: boolean }) {
    ...
    return (
      <div className="rounded-md border border-dashed border-neutral-300 bg-white p-6 text-sm text-neutral-600">
        <p className="font-medium text-neutral-800">
          Capteur détaché — aucune donnée récente.
        </p>
        <p className="mt-1">
          L&apos;historique des mesures reste consultable via la carte
          &laquo; Historique des capteurs &raquo; ci-dessous. Pour reprendre
          la collecte, associez un nouveau capteur ESP32 à cette parcelle.
        </p>
      </div>
    );
  }
  return (
    <div className="rounded-md border border-dashed border-neutral-300 bg-white p-6 text-sm text-neutral-600">
      <p className="font-medium text-neutral-800">
        Aucune donnée de capteur pour le moment.
      </p>
      <p className="mt-1">
        Si vous venez d&apos;associer un ESP32, la première mesure arrive dans
        les 15 minutes. Vérifiez l&apos;association du capteur ci-dessus.
      </p>
    </div>
  );
}
...
function RequestDiagnosticSlot() {
  return (
    <button
      type="button"
      disabled
      title="Disponible bientôt (KAT-07)"
