    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-neutral-500">
          ...
          Livraison vers <span className="font-medium">{order.delivery_region}</span>
        </p>
      </div>
      ...
      <div className="rounded-lg border border-neutral-200 bg-white">
        <table className="min-w-full divide-y divide-neutral-200 text-sm">
          <thead className="bg-neutral-50">
            <tr>
              {["Annonce", "Quantité", "Prix unitaire", "Sous-total", "Statut"].map((h) => (
                <th
                  key={h}
          ...
          <tbody className="divide-y divide-neutral-100">
            {order.items.map((it) => (
              <tr key={it.id}>
                ...
                <td className="px-4 py-3">{Number(it.quantity_kg).toFixed(2)} kg</td>
                <td className="px-4 py-3">{Number(it.unit_price_mad).toFixed(2)} MAD</td>
      ...
      <aside className="grid gap-4 md:grid-cols-2">
        <div className="rounded-lg border border-neutral-200 bg-white p-4">
          <h2 className="mb-2 text-sm font-semibold text-neutral-900">Totaux</h2>
